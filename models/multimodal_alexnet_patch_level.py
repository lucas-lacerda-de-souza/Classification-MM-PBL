"""
Multimodal AlexNet Classifier for MM and PBL
-----------------------------------------------------
...
Description:
    ...
    The model integrates information from histopathological image patches and clinicopathologic + nuclear morphometric features obtained from each case.
    Image embeddings are extracted using an AlexNet convolutional backbone, while tabular clinical/morphometric data are processed by a fully-connected network.
    Both branches are fused in a late-fusion multimodal architecture to improve diagnostic accuracy and robustness.

    The network is trained using patient-level stratification to prevent data leakage, evaluated on an internal validation/test split, and further assessed using
    external validation cohorts. The script outputs classification metrics, confusion matrix, ROC curve, and detailed explainability-ready probability scores.
...
def main():
    ...
    val_dir = "data/val"
    test_dir = "data/test"
    results_dir = "results/multimodal_alexnet"
